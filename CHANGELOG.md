...
- **At least one other G7 Pro variant couldn't enter vendor mode at
  all.** Reported by a community member testing a "Tri-mode" G7 Pro
  (pre-production unit): the handshake was sent successfully, but
  `enter_vendor_mode()` only ever checked two vendor-identity PIDs, and
  this hardware's is a different one. No timeout, however long, was
  ever going to help -- it was watching the wrong two addresses.
  `g7ctl` now also recognizes `3537:1003`, confirmed on the reporter's
  own hardware (a real config round trip, not just a matching USB
  descriptor shape). The `udev` rule needed for raw access without root
  didn't list it either -- added. Purely additive: no behavior change
  for the standard `109b`/`109c` identities.
