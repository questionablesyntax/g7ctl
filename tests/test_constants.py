...
question, via the vendor-mode PID rather than a CMD=0x01 sweep. See
constants.py's own comment on VARIANT_NAMES for the reasoning.
"""
import unittest
...
class IdentifyVariantTest(unittest.TestCase):
    def test_known_vendor_pids_resolve_to_their_confirmed_names(self):
        self.assertEqual(constants.identify_variant(constants.PID_VENDOR), "Shadow Ember")
        self.assertEqual(constants.identify_variant(constants.PID_VENDOR_TRIMODE), "White Trimode")
        self.assertEqual(constants.identify_variant(constants.PID_VENDOR_ZZZ), "Zenless Zone Zero")
    ...
    def test_dongle_and_xinput_pids_are_not_variant_lookups(self):
        # Deliberately keyed on vendor PID only -- a dongle PID is its own
        # wired counterpart + 1 wherever confirmed, not a separate lookup
        # here, and XInput/native PIDs aren't independently confirmed to
        ...
        self.assertIsNone(constants.identify_variant(constants.PID_XINPUT))
