...
| **EXPERIMENTAL** -- G7 SE | `3537:1010` | Reported 2026-08-20 -- genuinely different hardware, not just another PID under a known shape: `bNumInterfaces` is **3, not 2**. Interfaces 0 and 1 match every entry above (interrupt, then the isochronous alt-pair); interface 2 is new, a **bulk** endpoint pair nothing else in this project has. `bcdDevice` `6.33`. iProduct is also new (`"GameSir-G7 SE Controller for Xbox"`), matching neither the XInput idle string nor the vendor-mode one above. No handshake was sent to reach this capture -- it is the default idle state, unlike every other vendor-mode row here. `find_xinput_device()` finds nothing for this PID (confirmed: a plain `read-state` attempt reports "No device found at 3537:100a"), and Menu+Share has no observed effect on this hardware either. **Not confirmed as usable** -- added to `VENDOR_PID_CANDIDATES` only to test whether `find_writable_device()` recognizes it (read-only; no write path touches this PID). Whether this hardware speaks the same register-read protocol at all is unknown. |
